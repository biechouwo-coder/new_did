...
# 读取去重后的PSM匹配数据（缩尾版）
df = pd.read_excel('CEADs_PSM_matched_data_winsorized_dedup.xlsx')
...
plt.tight_layout()
plt.savefig('PSM_DID_coefficients_comparison_winsorized.png', dpi=300, bbox_inches='tight')
print("\n系数对比图已保存: PSM_DID_coefficients_comparison_winsorized.png")

# 保存回归结果到Excel
with pd.ExcelWriter('PSM_DID_regression_results_winsorized.xlsx', engine='openpyxl') as writer:
    # 创建详细结果表
...
print("\n回归结果已保存到: PSM_DID_regression_results_winsorized.xlsx")

print(f"\n{'=' * 80}")
print("PSM-DID分析完成！")
print(f"{'=' * 80}")
print(f"\n输出文件:")
print(f"1. PSM_DID_regression_results_winsorized.xlsx - 完整回归结果")
print(f"2. PSM_DID_coefficients_comparison_winsorized.png - 系数对比图")
