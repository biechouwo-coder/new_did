...
# 读取去重后的PSM匹配数据（4变量版本）
df = pd.read_excel('results_4vars/CEADs_PSM_matched_data_dedup_4vars.xlsx')
...
# 定义控制变量 - 只使用4个变量
control_vars = ['ln_pgdp', 'ln_pop_density', 'industrial_advanced', 'ln_fdi_openness']
...
plt.tight_layout()
plt.savefig('results_4vars/DID_4vars_coefficients_comparison.png', dpi=300, bbox_inches='tight')
print("\n系数对比图已保存: results_4vars/DID_4vars_coefficients_comparison.png")

# 保存回归结果到Excel
with pd.ExcelWriter('results_4vars/PSM_DID_regression_results_4vars.xlsx', engine='openpyxl') as writer:
    # 创建详细结果表
...
print("\n回归结果已保存到: results_4vars/PSM_DID_regression_results_4vars.xlsx")

print(f"\n{'=' * 80}")
print("PSM-DID分析完成！")
print(f"{'=' * 80}")
print(f"\n输出文件:")
print(f"1. results_4vars/PSM_DID_regression_results_4vars.xlsx - 完整回归结果")
print(f"2. results_4vars/DID_4vars_coefficients_comparison.png - 系数对比图")
